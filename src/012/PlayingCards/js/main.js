import { denotePositionOfCards } from "./utils";
...
form.addEventListener('submit', function(evt) {
    ...
    if (names.length > 0) {
        ...
        names.forEach((player, index) => {
            container.innerHTML += `<div id="player-${index}"><h2>${player}</h2><aside><button data-id="player-${index}" data-sort="suit">♠️♥️♦️♣️</button><button data-id="player-${index}" data-sort="value">Ace-King</button></aside></div>`;
        });
...
container.addEventListener('click', function(evt) {
    evt.preventDefault();
    // Tasks:
    // 1) Determine which button was clicked (if any)
    // 2) Process the action associated with the button

    switch(evt.target.tagName) {
        case 'BUTTON':
            const btn = evt.target;
            // Look for the data-id and data-sort attributes
            const playerId = btn.dataset.id;
            const sortBy = btn.dataset.sort;
            console.log(playerId, sortBy);
            // Get all the cards for that player
